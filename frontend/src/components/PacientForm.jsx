...
export default function PacientForm({ onSaved, onCancel }) {
  ...
  const inputStyle = {
    ...
    color: 'var(--text-primary)', boxSizing: 'border-box', marginBottom: '12px', outline: 'none',
  }
  ...
  const handleAdresaChange = (field, value) => setForm(prev => ({ ...prev, [field]: value }))

  const handleSubmit = async e => {
    e.preventDefault()
    if (!validareCNP(form.cnp)) { setErrors({ cnp: ['CNP invalid — verificati numarul introdus.'] }); return }
    setLoading(true); setErrors({})
    try { await api.post('/pacienti/', form); onSaved() }
    catch (err) {
  ...
  return (
    <div style={{ maxWidth: '620px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '20px' }}>
        <button onClick={onCancel} style={{ background: 'none', border: 'none', color: 'var(--text-dim)', cursor: 'pointer', fontSize: '20px', padding: '0', lineHeight: 1 }}>←</button>
        <span style={{ fontSize: '15px', fontWeight: '600', color: 'var(--text-primary)' }}>Pacient nou</span>
      </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
            <div>
              ...
              <input name="nume" value={form.nume} onChange={handleChange} required style={inputStyle} placeholder="ex. Ionescu"
                onFocus={e => e.target.style.borderColor = 'var(--accent)'}
                onBlur={e => e.target.style.borderColor = 'var(--border)'} />
            </div>
            <div>
              <label style={labelStyle}>Prenume *</label>
              <input name="prenume" value={form.prenume} onChange={handleChange} required style={inputStyle} placeholder="ex. Maria"
                onFocus={e => e.target.style.borderColor = 'var(--accent)'}
                onBlur={e => e.target.style.borderColor = 'var(--border)'} />
            </div>
          </div>
          ...
          <input name="cnp" value={form.cnp} onChange={handleChange} required maxLength={13} style={inputStyle} placeholder="13 cifre"
            onFocus={e => e.target.style.borderColor = 'var(--accent)'}
            onBlur={e => e.target.style.borderColor = errors.cnp ? '#f87171' : 'var(--border)'} />
          {errors.cnp && <p style={errorStyle}>{errors.cnp[0] === 'pacient with this cnp already exists.' ? 'Acest CNP este deja inregistrat.' : errors.cnp[0]}</p>}

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
            <div>
              <label style={labelStyle}>Data nasterii *</label>
              <input name="data_nastere" type="date" value={form.data_nastere} onChange={handleChange} required style={inputStyle}
                onFocus={e => e.target.style.borderColor = 'var(--accent)'}
                onBlur={e => e.target.style.borderColor = 'var(--border)'} />
            </div>
            <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 16px' }}>
            <div>
              ...
              <input name="telefon" value={form.telefon} onChange={handleChange} style={inputStyle} placeholder="07xx xxx xxx"
                onFocus={e => e.target.style.borderColor = 'var(--accent)'}
                onBlur={e => e.target.style.borderColor = 'var(--border)'} />
            </div>
            <div>
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '8px' }}>
            ...
            <button type="submit" disabled={loading}
              style={{ padding: '9px 20px', fontSize: '13px', cursor: 'pointer', background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: '8px', opacity: loading ? 0.6 : 1 }}>
