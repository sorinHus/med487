...
function Badge({ status }) {
  ...
  return <span className={s.badge} style={{ background: st.bg, color: st.color }}>{st.label}</span>
}
...
function CalendarLunar({ selectedDate, onSelectDate, programariExistente }) {
  ...
  for (let i = 0; i < rows * 7; i++) {
    ...
    cells.push(dayNum < 1 || dayNum > lastDay.getDate() ? null : dayNum)
  }

  const zileOcupate = new Set((programariExistente || []).map(p => p.data_ora.slice(0, 10)))
  ...
  return (
    <div className={s.calLunar}>
      ...
      <div className={s.calLunar__grid}>
        {ZILE_SCURT.map(z => <div key={z} className={s.calLunar__ziHeader}>{z}</div>)}
...
/* ── Grid sloturi ── */
function GridSloturi({ data, oraSelectata, onSelectOra, excludeOra }) {
  ...
  useEffect(() => {
    if (!data) return
    setLoading(true); setSloturi(null); setError(null)
  ...
  // La editare, slotul original e mereu liber (excludeOra)
  const sloturiAfisate = sloturi.map(sl =>
    excludeOra && sl.ora === excludeOra ? { ...sl, liber: true } : sl
  )
  ...
  return (
    <div className={s.sloturi}>
      ...
      <div className={s.sloturi__grid}>
        {sloturiAfisate.map(({ ora, liber }) => {
          const selected = oraSelectata === ora
          return (
            <button type="button" key={ora} disabled={!liber}
              onClick={() => liber && onSelectOra(ora)}
              className={[s.slot, liber ? s['slot--liber'] : s['slot--ocupat'], selected ? s['slot--selected'] : ''].join(' ')}
              title={liber ? 'Liber' : 'Ocupat'}>
              {ora}
            </button>
          )
        })}
...
function ModalProgramare({ onClose, onSaved, programariExistente }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!selectedDate || !selectedOra) { setError('Selectati o zi si un slot orar.'); return }
    setSaving(true); setError(null)
    try {
      await api.post('/programari/', { ...form, data_ora: `${selectedDate}T${selectedOra}` })
      setSaving(false); onSaved()
    } catch { setError('Eroare la salvare. Verificati datele.'); setSaving(false) }
  }
...
/* ── Modal editare ── */
function ModalEditare({ programare, onClose, onSaved, programariExistente }) {
  const dataOraInit  = new Date(programare.data_ora)
  const dateStr      = formatDate(dataOraInit)
  const oraInit      = `${String(dataOraInit.getHours()).padStart(2,'0')}:${String(dataOraInit.getMinutes()).padStart(2,'0')}`

  const [selectedDate, setSelectedDate] = useState(dateStr)
  const [selectedOra, setSelectedOra]   = useState(oraInit)
  const [form, setForm] = useState({
    nume_pacient:    programare.nume_pacient    || programare.pacient_nume_complet || '',
    telefon_pacient: programare.telefon_pacient || '',
    email_pacient:   programare.email_pacient   || '',
    motiv:           programare.motiv           || '',
    durata_min:      programare.durata_min      || 20,
    status:          programare.status          || 'programat',
    medic:           programare.medic           || 1,
  })
  const [saving, setSaving] = useState(false)
  const [error, setError]   = useState(null)
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true); setError(null)
    try {
      await api.patch(`/programari/${programare.id}/`, {
        ...form,
        data_ora: `${selectedDate}T${selectedOra}`,
      })
      setSaving(false); onSaved()
    } catch { setError('Eroare la salvare. Verificati datele.'); setSaving(false) }
  }
  ...
  return (
    <div className={s.modalOverlay}>
      <div className={s.modalBox} style={{ maxWidth: 560 }}>
        <div className={s.modalHeader}>
          <span className={s.modalTitle}>Editare programare</span>
          <button onClick={onClose} className={s.modalClose}>×</button>
        </div>
        {error && <div className={s.modalError}>{error}</div>}
        <form onSubmit={handleSubmit}>
...
/* ── Rand programare ── */
function RandProgramare({ p, onStatusChange, onEdit, index }) {
  ...
  return (
    <div className={s.rand}>
      ...
      <div className={s.rand__avatar} style={{ background: getAvatarColor(nume) }}>{getInitials(nume)}</div>
      ...
      <div className={s.rand__actions}>
        ...
        <button onClick={() => onEdit(p)} title="Editeaza" className={s.btnEdit}>✎</button>
        {p.status === 'programat' && (
    ...
    <div>
                  ...
                  <div>
                    {prog.map((p, index) => (
                      <RandProgramare key={p.id} p={p} onStatusChange={fetchProgramari}
                        onEdit={setEditProgramare} index={index} />
                    ))}
                  </div>
                )}
              </div>
      ...
      {editProgramare && (
        <ModalEditare
          ...
          onSaved={() => { setEditProgramare(null); fetchProgramari() }}
          programariExistente={programari} />
      )}
    </div>
  )
